...
- Logging goes through the shared Winston `logger` (`electron/logger.ts`), writing to `%APPDATA%/OnliCert Manager/logs` — not `console.log`, and not `<install-dir>` (that caused an `EPERM` regression previously, see `PROJECT_HISTORY.md`).
